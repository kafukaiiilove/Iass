  ...
  <div>
    ...
    <el-dialog 
      ...
      <el-upload
        class="upload-demo"
        drag
        :action="$baseUrl + '/manager/a14/markdown/upload'"
...
export default {
  ...
  methods: {
    load(pageNum) {
      ...
      this.$request.get('/manager/a14/selectPage', { params }).then(res => {
      ...
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          ...
          try {
            ...
            const url = this.form.id ? '/manager/a14/update' : '/manager/a14/add'
    ...
    handleDelete(id) {
      ...
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(() => {
        this.$request.delete('/manager/a14/delete/' + id).then(res => {
          if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(() => {
        this.$request.delete('/manager/a14/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {
    ...
    customUpload(options) {
      ...
      this.$request({
        url: '/manager/a14/markdown/upload',
        method: 'post',
        data: formData,
        headers: {
    ...
    handleViewMarkdown(row) {
      this.$request.get('/manager/a14/selectById/' + row.id).then(res => {
        if (res.code === '200') {
          const data = res.data;
          let md = '';
